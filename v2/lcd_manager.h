...
// Thin wrapper around LiquidCrystal that redraws only the characters
// which actually changed, avoiding lcd.clear() calls (which cause a
// visible flicker on every refresh) while still refreshing at a
// practical rate.
class LCDManager {
public:
    LCDManager();
